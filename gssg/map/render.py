...
from gssg.utils.utils import devF, devI
...
class Renderer:
    ...
    def __init__(self, args):
        ...
        self.renderer_normal_threshold = np.cos(np.deg2rad(args.renderer_normal_threshold))
    ...
    def render(self, viewpoint_camera: Camera, gaussian_data, tile_mask=None, semantic_id=None):
        ...
        self.raster_settings = GaussianRasterizationSettings_depth(
            ...
            tanfovx=tanfovx,
            tanfovy=tanfovy,
            bg=self.bg_color if semantic_id is None else devF(torch.tensor([1, 1, 1])),
        ...
        try:
            ...
            cov3D_precomp = None
            colors_precomp = None
            semantics = gaussian_data.get("semantics", None)
            if semantic_id is not None and semantics is not None:
                mask = (semantics == semantic_id).squeeze()
            ...
            save_path = repo_path("scripts", "debug", "data.pt")
            logging.error(f"[RENDERER] Error: {e} - SAVED to {save_path}")
            torch.save(gaussian_data, save_path)
            raise  # don't fall through to the rasterizer with undefined locals
