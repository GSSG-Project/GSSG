...
from gssg.utils.general_utils import devF, devI, quaternion_from_axis_angle
...
def rot_compare(prev_rot: np.array, curr_rot: np.array):
    ...
    cos_theta = np.clip((np.trace(rot_diff) - 1) / 2, -1.0, 1.0)
    rad_diff = np.arccos(cos_theta)
    theta_diff = np.rad2deg(rad_diff)
    return rad_diff, theta_diff  # (radians, degrees) — compare against radian thresholds
...
def transform_map(map: torch.tensor, transform: torch.tensor):
    ...
    if C == 3:
        # One flat (H*W,3)@(3,3) matmul + translation instead of H*W batched 4x4 matmuls.
        flat = map.reshape(-1, 3)
        out = flat @ transform[:3, :3].T + transform[:3, 3]
        return out.reshape(H, W, 3)
    transform_expand = transform.unsqueeze(0).unsqueeze(0).expand(H, W, -1, -1)
    new_map = torch.matmul(transform_expand, homogeneous(map).unsqueeze(-1)).squeeze()[..., :C]
    return new_map
...
def compute_vertex_map(depth, K):
    ...
    vertex = (
        torch.stack([(i - cx) / fx, (j - cy) / fy, torch.ones_like(i)], -1).to(device) * depth
    )  # [h, w, 3]
    return vertex
...
def compute_normal_map(vertex_map):
    ...
    normal = torch.linalg.cross(img_dy.view(-1, 3), img_dx.view(-1, 3))
...
def color_value(value, invalid_mask=None, min_depth=0, max_depth=0, color_map=cv2.COLORMAP_JET):
    if value.shape[0] > 3:
...
def compute_rot(init_vec, target_vec):
    axis = torch.linalg.cross(init_vec, target_vec)
...
def save_resolved_config(args, save_path):
    """Persist the fully-resolved run config (post `parent:` inheritance) as plain
    YAML that round-trips through `yaml.safe_load`.

    `yaml.dump(args)` of a GroupParams emits a `!!python/object` tag that only
    `unsafe_load` can read; `vars(args)` is the flat resolved key->value mapping
    (leading `_` already stripped) and dumps as a clean, re-runnable config
    (`parent: None` short-circuits inheritance on reload). Used by both the
    single- and multi-process entrypoints."""
    os.makedirs(save_path, exist_ok=True)
    resolved = dict(vars(args))
    path = os.path.join(save_path, "config.yaml")
    try:
        with open(path, "w") as f:
            yaml.safe_dump(resolved, f, default_flow_style=False, sort_keys=True)
    except yaml.representer.RepresenterError:
        # A stray non-native value (numpy, Path, ...) — stringify those and retry so
        # the config still persists rather than aborting the run.
        safe = {k: (v if _yaml_native(v) else str(v)) for k, v in resolved.items()}
        with open(path, "w") as f:
            yaml.safe_dump(safe, f, default_flow_style=False, sort_keys=True)


def prepare_cfg(args):
    data_path = args.source_path
    save_path = args.save_path
    save_resolved_config(args, save_path)

    if args.parent != "None":
        os.system("cp {} {}".format(args.parent, os.path.join(args.save_path, "base.yaml")))
    os.system(
        "cp {} {}".format(
    ...
    with open(os.path.join(save_path, "cfg_args"), "w") as f:
        cfg_args = "Namespace(gpu_device='cuda', eval=False, images='images', model_path='{}', resolution=-1, sh_degree={}, source_path='{}', white_background=False)".format(
...
def merge_ply(ply0_path, ply1_path, save_path, include_confidence):
    ...
    for i, _data in enumerate(ply0_data):
...
def eval_ate(pose_estimate, pose_gt):
    def align(model, data):
        ...
        trans_error = numpy.sqrt(numpy.sum(numpy.multiply(alignment_error, alignment_error), 0)).A[
            0
        ]
...
def build_vertex_pyramid(depth, pyramid_builder, K):
    ...
    for i in range(len(depth_pyramid)):
        ...
        K_downscale[2, 2] = 1.0
        vertex_pyramid.append(
            compute_vertex_map(depth_pyramid[i].reshape(H_sub, W_sub, 1), K_downscale)
        )
...
# --- run_mp tensor staging for Tegra (no CUDA IPC) ------------------------------
# torch.multiprocessing shares CUDA tensors across processes via CUDA IPC, which
# Tegra/Jetson (integrated memory) does NOT support — run_mp.py otherwise dies with
# "CUDA error: invalid argument" in rebuild_cuda_tensor. On Tegra we stage every
# cross-process payload through CPU; the consumer restores it to GPU. dGPU desktops
# (e.g. RTX 4090) support CUDA IPC, so all of this is a no-op there (zero-copy kept).
NO_CUDA_IPC = os.path.exists("/etc/nv_tegra_release")

# Frame-preprocess maps the mapper consumes on the GPU. semantic_map is int32, so
# the restore is dtype-preserving (.to) — NOT devF, which would float-cast it.
_FRAME_MAP_GPU_KEYS = (
    "depth_map",
    "color_map",
    "normal_map_c",
...
def _tensors_to_cpu(obj):
    """Recursively detach + move every tensor in a dict/list/tuple to CPU. Other
    objects (ints, Camera, ...) pass through unchanged."""
...
def frame_tensors_to(frame, device):
    """Move EVERY tensor attribute of a Camera frame to `device` in place. Unlike
    move_to_gpu/move_to_cpu (which only touch original_image/original_depth), this
    also moves world_view_transform / projection_matrix / full_proj_transform /
    camera_center — plain attributes that nn.Module.to() and CUDA IPC both choke on.
    Needed to cross a mp.Queue on Tegra (no CUDA IPC)."""
    if frame is None:
        return frame
    for name, val in list(vars(frame).items()):
        if torch.is_tensor(val):
            setattr(frame, name, val.detach().to(device))
    return frame


def stage_map_input_for_queue(map_input):
    """Tegra: move a tracker->mapper frame payload to CPU so it crosses a mp.Queue
    without CUDA IPC. The whole Camera frame (all tensors) goes to CPU; the mapper
    restores the GPU maps (set_input) and the frame (run). No-op on dGPU."""
    if not NO_CUDA_IPC:
        return map_input
    out = {}
    for k, v in map_input.items():
        if k == "frame":
            out[k] = frame_tensors_to(v, "cpu")
        else:
            out[k] = _tensors_to_cpu(v)
    return out


def restore_frame_map_to_gpu(frame_map):
    """Tegra: move the mapper's GPU-consumed maps back to CUDA (dtype-preserving).
    No-op on dGPU."""
    if not NO_CUDA_IPC:
        return
    for k in _FRAME_MAP_GPU_KEYS:
        v = frame_map.get(k)
        if torch.is_tensor(v) and not v.is_cuda:
            frame_map[k] = v.to("cuda", non_blocking=True)


def stage_tensors_to_cpu(obj):
    """Tegra: CPU-stage any nested dict/list/tuple of tensors for a mp.Queue.
    No-op on dGPU. (Camera frames and GaussianPointCloud are handled by callers.)"""
    return _tensors_to_cpu(obj) if NO_CUDA_IPC else obj
...
def bilateralFilter_torch(
    ...
    for i in range(-radius, radius + 1):
        for j in range(-radius, radius + 1):
            ...
            color_weight = -(
                (depth - depth_pad[radius + i : radius + i + h, radius + j : radius + j + w]) ** 2
            ) / (2 * (sigma_color**2))
            weight = torch.exp(spatial_weight + color_weight)
            weight_mask = depth_pad[radius + i : radius + i + h, radius + j : radius + j + w] != 0
            weight = weight * weight_mask
            weight_sum += weight
            pixel_sum += (
                weight * depth_pad[radius + i : radius + i + h, radius + j : radius + j + w]
            )
...
def bbox_filter(local_xyz, total_xyz, padding=0.05):
    # An empty local set has no bounding box (min/max over dim 0 would raise on a
    # zero-size tensor); nothing of total_xyz can be inside it -> all-False mask.
    if local_xyz.shape[0] == 0 or total_xyz.shape[0] == 0:
        return torch.zeros(total_xyz.shape[0], dtype=torch.bool, device=total_xyz.device)
    local_min = local_xyz.min(dim=0)[0] - padding
    local_max = local_xyz.max(dim=0)[0] + padding
    inbbox_mask = (total_xyz > local_min).all(dim=-1) & (total_xyz < local_max).all(dim=-1)
